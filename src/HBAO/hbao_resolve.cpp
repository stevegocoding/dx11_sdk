...
HRESULT c_hbao_resolve_component::recompile_effect()
{
    ...
    m_resolve_depth_tech = m_effect_params_table->get_technique_by_name("resolve_depth_tech");
    m_linearize_depth_tech = m_effect_params_table->get_technique_by_name("linearize_depth_tech");
...
void c_hbao_resolve_component::apply_linearize_depth(const texture_2d_ptr& input_depth, texture_2d_ptr& output_depth)
{
    texture_2d_ptr dest_rt;
    dest_rt = m_need_upsample? m_src_rt_depth : m_rt_resolved_depth; 

    m_render_sys_context->get_d3d11_device_context()->RSSetViewports(1, &m_in_vp);
    ID3D11RenderTargetView *rtv[] = {dest_rt->get_rtv()};
    m_render_sys_context->get_d3d11_device_context()->OMSetRenderTargets(1, rtv, NULL);

    m_tex_depth_sr->SetResource(input_depth->get_srv());
    m_linearize_depth_tech->GetPassByIndex(0)->Apply(0, m_render_sys_context->get_d3d11_device_context());
    ...
    output_depth = dest_rt;
}

void c_hbao_resolve_component::apply_resolve_depth(const texture_2d_ptr& input_depth)
{
    HRESULT hr; 

    PIX_EVENT_BEGIN_TECHNIQUE(resolve_depth_tech);
    PIX_EVENT_SET_RENDER_TARGETS()
    ID3D11RenderTargetView *rtvs[] = {m_rt_resolved_depth->get_rtv()};
	...
	m_tex_depth_sr->SetResource(input_depth->get_srv());
	m_resolve_depth_tech->GetPassByIndex(0)->Apply(0, m_render_sys_context->get_d3d11_device_context());

    m_render_sys_context->get_d3d11_device_context()->Draw(3, 0);
    PIX_EVENT_END_TECHNIQUE(resolve_depth_tech);
	
    PIX_EVENT_SET_EFFECT_SHADER_RESOUCE()
    hr = m_effect_params_table->unbind_shader_resource(m_tex_depth_sr);
    hr = m_effect_params_table->apply_technique(m_resolve_depth_tech);
	m_render_sys_context->get_d3d11_device_context()->OMSetRenderTargets(0, NULL, NULL);
}
